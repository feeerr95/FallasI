...
class Robot3DTroubles(KnowledgeEngine):
    ...
    def init_Trouble(self):
        next_step = ['Problemas con la primer capa', 'Hilos rodeando la pieza', 'Mala definición',
                     'Pieza desfasada', 'Pieza quedó a medio hacer', 'Ausencia de filamento al inicio de la impresión']
        update(next_step)
    ...
    def bad_Definition_Trouble(self):
        next_step = ['La pieza tiene burbujas explotadas', 'La pieza tiene agujeros o falta de material entre las capas',
                     'Las poleas están ajustadas', 'Las poleas no están ajustadas']
        update(next_step)
        # Mala definición
    ...
    def level_Trouble(self):
        next_step = ['La velocidad de impresión no es la correcta',
    ...
    def high_Speed_Trouble(self):
        next_step = [
            'Configurar bien la velocidad de impresión según el modelo']
        update(next_step)
        # ----------------------------------------------------

    @Rule(Trouble(error='La velocidad de impresión es la correcta'))
    def speed_Trouble(self):
        next_step = ['La impresora tiene cama caliente',
    ...
    def no_Hot_Bed_Trouble(self):
        next_step = ['Aplique algún adhesivo sobre la cama',
                     'No aplique algún adhesivo sobre la cama']
        update(next_step)
        # La impresora no tiene cama caliente
    ...
    def threads_Trouble(self):
        next_step = ['La temperatura está configurada correctamente',
    ...
    def temperature_Trouble(self):
        next_step = ['La velocidad de impresión es la correcta.',
                     'La velocidad de impresión no es la correcta.']
        update(next_step)
    ...
    def out_Of_Phase_Trouble(self):
        next_step = ['Las poleas están ajustadas',
                     'Las poleas no están ajustadas']
        update(next_step)
    # Pieza desfasada
    ...
    def too_Much_Plastic_Trouble(self):
        next_step = ['Los soportes fueron configurados',
                     'Los soportes no fueron configurados']
        update(next_step)
    ...
    def plastic_Trouble(self):
        next_step = ['El carrete no tiene filamento',
                     'Hay excedente de plástico entre el extrusor y barrel', 'El nozzle está tapado']
        update(next_step)
    ...
    def excess_Plastic_Trouble(self):
        next_step = [
            'La temperatura del barrel excede de la correcta, configurar bien la temperatura del mismo']
        update(next_step)
        # ----------------------------------------------------

    @Rule(Trouble(error='El nozzle está tapado'))
    def block_Nozzle_Trouble(self):
        next_step = ['La temperatura del barrel está por debajo de la esperada',
    ...
    def tight_Nozzle_Trouble(self):
        next_step = ['El barrel está desgastado',
                     'El barrel no está desgastado']
        update(next_step)
